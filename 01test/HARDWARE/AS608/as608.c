...
//				else 
//				{
//					Voice_play("对比失败，请重新录入指纹");
//					printf("add.t0.txt=\"对比失败，请重新录入指纹\"");
//					printf(sendEnd);
//					delay_s(3);
//					mymemset(str,0,50);
//					
...
{	
	...
	while(1)
	{
		switch (processnum)
		{
			case 0:
				i++;
				ensure=PS_GetImage();//请按手指
				if(ensure==0x00) //获取到指纹
				{
					ensure=PS_GenChar(CharBuffer1);//生成特征
					if(ensure==0x00)
					{
...
//					else 
//						OLED_Show_Str(0, 33,"Finger no I",12);//指纹有误				
				}
//				else 
//					//OLED_Show_Str(0, 20,"Finger X",12);//指纹未获取						
				break;
			case 1:
				i++;
				ensure=PS_GetImage();//再按一次手指
				if(ensure==0x00) 
				{
					ensure=PS_GenChar(CharBuffer2);//生成特征			
					if(ensure==0x00)//指纹正确
...
void Del_FR(void)
{
	...
	str=mymalloc(SRAMIN,30);
	//Voice_play("删除指纹");
	printf("delete.t0.txt=\"删除指纹\"");
	printf(sendEnd);
	delay_s(2);	
	//Voice_play("请输入指纹ID");
	printf("delete.t0.txt=\"请输入指纹ID\"");
	printf(sendEnd);
	delay_s(2);
	printf("page ID");
	printf(sendEnd);
...
//			{
//				mymemset(str,0,50);
//				sprintf(str,"%d号同学签到成功",seach.pageID);
//				Voice_play(str);
//				myfree(SRAMIN,str);
//				
